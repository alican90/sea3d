	...
	public class Bounds
	{
		...
		public static function getObjectContainerBounds(container:ObjectContainer3D, worldBased:Boolean = true, ignorePosition:Boolean = false):void
		{
			...
			if (worldBased) {
				...
				if (ignorePosition)
				{
					var t:Matrix3D = new Matrix3D( Matrix3DUtils.RAW_DATA_CONTAINER );
					t.copyFrom( container.sceneTransform );
					t.position = new Vector3D();					
					
					transformContainer(b, c, t);
				}
				else transformContainer(b, c, container.sceneTransform);
