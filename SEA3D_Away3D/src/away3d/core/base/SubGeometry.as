package away3d.core.base
{
	...
	public class SubGeometry extends SubGeometryBase implements ISubGeometry
	{
		...
		public function activateSecondaryUVBuffer(index:int, stage3DProxy:Stage3DProxy):void
		{
			...
			if (_secondaryUvsInvalid[contextIndex] && _secondaryUvs) {
				_secondaryUvBuffer[contextIndex].uploadFromVector(_secondaryUvs, 0, _numVertices);
				_secondaryUvsInvalid[contextIndex] = false;
			}
			
			context.setVertexBufferAt(index, _secondaryUvs ? _secondaryUvBuffer[contextIndex] : _uvBuffer[contextIndex], 0, Context3DVertexBufferFormat.FLOAT_2);
		}
		...
		arcane function invalidateVertexNormalData(vertexNormals : Vector.<Number>) : void
		{
		...
		public function fromVectors(verts:Vector.<Number>, uvs:Vector.<Number>, normals:Vector.<Number>, tangents:Vector.<Number>, uvs2:Vector.<Number>=null):void
		{
			updateVertexData(verts);
			
			if (uvs && uvs.length) updateUVData(uvs);	
			else autoGenerateDummyUVs = true;
			
			if (normals && normals.length) 	updateVertexNormalData(normals);	
			else autoDeriveVertexNormals = true;
			
			if (tangents && tangents.length) updateVertexTangentData(tangents);	
			else autoDeriveVertexTangents = true;
			
			if (uvs2 && uvs2.length) updateSecondaryUVData(uvs2);			
		}
